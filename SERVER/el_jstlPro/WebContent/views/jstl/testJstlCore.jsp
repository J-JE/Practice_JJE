...
<body>
	...
	* 변수 선언 (c:set)
	...
	<h2>c:set 태그 : 배열로 사용할 문자열을 선언</h2>
	<c:set var="colors">
		red, yellow, green, orange, blue, magenta
	</c:set>
	
	colors 값 확인 : 	${colors}<br>
	...
	* 변수 출력 (c:out)
	데이터를 출력할 때 사용하는 태그
	&lt;, &gt;, & 특수문자를 자동으로 
	이스케이프 시퀀스(escape sequence)로 변환 가능.
	</pre>
	
	<c:out value="core 라이브러리의 <out> 태그는 값을 화면에 출력하는 태그이다."/><br>
	
	<c:out value="<h2>데이터출력</h2>" escapeXml="false"/><br><%--태그 역할을 함 --%>
	<c:out value="<h2>데이터출력</h2>" escapeXml="true"/><br><%--태그 역할을 못함 --%>
	
	<h2>이스케이프 시퀀스 : <, >, & 등의 특수문자를 &lt;, &gt; &amp;로 바꿔서 인식시킨다.</h2>
	...
	<c:if test="${value1 + 0 >= value2 + 0}">
		<h3>value1이 큽니다. : <c:out value="${value1 }"/> </h3>
	</c:if>
	
	<c:if test="${value1 + 0 < value2 + 0}">
		<h3>value2가 큽니다. : <c:out value="${value2 }"/> </h3>
	</c:if>
	...
	<h2>c:forEach태그 : 배열 또는 컬렉션 연속처리에 for ~ each문 처럼 사용 가능하다.</h2>
	<c:forEach var="color" items="${colors }" varStatus="st">
		<font color="${ color }">${ st.index } : 글자색 ${ color }</font><br>
	</c:forEach>
	
	
	<h2>c:forTokens 태그 : 문자열을 토큰으로 분리처리 할 때 사용</h2>
	<pre>
	...
	<h3>여러 개의 토큰 문자를 가지고 분리할 수 도 있다.</h3>
	<c:forTokens items="yellow-blue*pink/red green" delims="*/ -" var="color">
		${ color }<br>
	</c:forTokens>
